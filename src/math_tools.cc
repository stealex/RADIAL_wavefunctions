...
void math_tools::GLNodesAndWeights(int nPoints){
    ...
    for (int i = 0; i < nPoints; i++){
        ...
        for (int j = 0; j < 20; j++){
            ...
            fpx = nPoints*(glNodes[i]*fx[nPoints] - f1x[nPoints-1])/(std::pow(glNodes[i],2) -1.);
...
void math_tools::ComputeScreeningFunction(int nPoints, double xMax, int nMaxCoeff, bool dump){
...
void math_tools::ComputeExpansionCoefficients(std::vector<double> &coeffs){
    ...
    for (int m = 2; m < nCoeffs; m++){
        ...
        double sum = 0.0;
        for (int n = 1; n < m-2; n++){
            sum += coeffs[m-n] *((n+1.)*coeffs[n+1] - 2.*(n+4.)*coeffs[n] + (n+7.)*coeffs[n-1]);
        }
